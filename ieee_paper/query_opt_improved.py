...
def create_schedule_improved(temp_table_matrix, temp_relation_reduce, edge_start, edge_end, manipulator, times): ### manipulator is used for m
	if(times == 0):
		return
	edge = min(temp_relation_reduce, key=temp_relation_reduce.get)
	temp_relation_reduce.pop(edge, None)
	edge = edge[::-1]
	temp_relation_reduce.pop(edge, None)
	rel_to = edge[0]
	rel_from = edge[2]
	temp_rand = randint(0,100) % 2
	if( manipulator == 1 or temp_rand == 0):
		temp_table_matrix[int(rel_to)][int(rel_from)] = str(edge_end)  ## single edged
		edge_end = edge_end - 1
		temp_table_matrix[int(rel_from)][int(rel_to)] = "0"
	elif( manipulator == 2 or temp_rand == 1):
		temp_table_matrix[int(rel_from)][int(rel_to)] = str(edge_start)  ## double edged
		edge_start = edge_start + 1
		temp_table_matrix[int(rel_to)][int(rel_from)] = str(edge_end)
		edge_end = edge_end - 1
	times -= 1
	create_schedule_improved(temp_table_matrix, temp_relation_reduce, edge_start, edge_end, manipulator, times)
...
#	print table_matrix
	...
	with open('relation_reduce.json', 'r+') as f:    ### contais the reduction fraction of tables when semi-joins is applied ###
                temp_relation_reduce = json.load(f)
	create_schedule_improved(temp_table_matrix, temp_relation_reduce, edge_start, edge_end, 0, 3)
...
while(change > 5):
		...
		for x in range(40 + n):
			...
			while(y < (40 + n)):
				...
				while( j < len(temp_sched) ):
					...
					while( k < len(temp_sched) and temp_sched[k] != ":" and temp_sched[k] != "." ):# extrcts joins 4m sched
						k = k + 1
					...
					#		k = k + 1
						...
						j = len(temp_sched)
					#### checkin if d newly formed schedule is better thn its parent schedule & replacing accordingly ####
	...
	for x in range(40 + n):
		...
		if(choice == 1):
			...
			edge_end = 2*(n-1)
			temp_table_matrix = table_matrix
			with open('relation_reduce.json', 'r+') as f:    ### contais the reduction fraction of tables when semi-joins is a
		                temp_relation_reduce = json.load(f)
		        create_schedule_improved(temp_table_matrix, temp_relation_reduce, edge_start, edge_end, 2, 3)

		#	create_schedule(schedule_root[x], -1, temp_table_matrix, edge_start, edge_end, 2) #### creates d sched 4 semi-joins ### 
													  #### "2" in d last parameter says ####
			...
			edge_end = 2*(n-1)
			temp_table_matrix = table_matrix
			with open('relation_reduce.json', 'r+') as f:    ### contais the reduction fraction of tables when semi-joins is appl
		                temp_relation_reduce = json.load(f)
		        create_schedule_improved(temp_table_matrix, temp_relation_reduce, edge_start, edge_end, 1, 3)

#			create_schedule(schedule_root[x], -1, temp_table_matrix, edge_start, edge_end, 1) #### creates d sched 4 semi-joins ### 
													  #### "1" in d last parameter says ###
			...
			if(count == 1):
				...
				for i in range(n):
					table_matrix[last_semijoin][i] = "0"
					table_matrix[i][last_semijoin] = "0"
			with open('relation_reduce.json', 'r+') as f:    ### contais the reduction fraction of tables when semi-joins is appli
		                temp_relation_reduce = json.load(f)
		        create_schedule_improved(temp_table_matrix, temp_relation_reduce, edge_start, edge_end, 0, 3)
