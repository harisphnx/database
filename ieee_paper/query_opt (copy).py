...
for x in range(40 + n):
	...
	j = 0
	while( i < len(temp_sched) ):
		while(temp_sched[j] != ":" and temp_sched[j] != "." and j < len(temp_sched) ):   ### extracts joins from scedule ###
			j = j + 1
		temp_join = temp_sched[i:j]
		#print temp_join
		j = j + 2
		...
		table_from = int(temp_join[2])
		cost = cost + ( float(temp_table_size[table_to]) * float(trans_cost[temp_join]) )   ### calculates cost ###
		temp_table_size[table_from] = str(float(relation_reduce[temp_join]) * float(temp_table_size[table_from]))  ### updates size ###
															   ### of table     ###
	cost = cost + int(full_reduce_cost[str(table_from)])'''
for x in range(40 + n):
        temp_sched = schedule[x]                    ### takes schedule one by one ###
	cost =  calc_cost(temp_sched, table_size)
